...
public final class ViewDraugHelper {
     ...
     */
    public static final int STATE_IDLE = DrawerLayout.STATE_IDLE;

    /**
     * A view is currently being dragged. The position is currently changing as a result
     * of user input or simulated user input.
     */
    public static final int STATE_DRAGGING = DrawerLayout.STATE_DRAGGING;

    /**
     * A view is currently settling into place as a result of a fling or
     * predefined non-interactive motion.
     */
    public static final int STATE_SETTLING = DrawerLayout.STATE_SETTLING;
     ...
     */
    public static ViewDraugHelper create(ViewGroup forParent, Callback cb) {
        return new ViewDraugHelper(forParent.getContext(), forParent, cb);
    }

    public static ViewDraugHelper create(ViewGroup forParent, Callback cb, Interpolator interpolator) {
        return new ViewDraugHelper(forParent.getContext(), forParent, cb, interpolator);
    }

    public static ViewDraugHelper create(ViewGroup forParent, float sensitivity, Callback cb, Interpolator interpolator) {
        final ViewDraugHelper helper = create(forParent, cb, interpolator);
     ...
     * @return a new ViewDragHelper instance
     */
    public static ViewDraugHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final ViewDraugHelper helper = create(forParent, cb);
        helper.mTouchSlop = (int) (helper.mTouchSlop * (1 / sensitivity));
        return helper;
    }
     ...
     * @param forParent Parent view to monitor
     */
    private ViewDraugHelper(Context context, ViewGroup forParent, Callback cb) {
        this(context, forParent, cb, sInterpolator);
    }

    private ViewDraugHelper(Context context, ViewGroup forParent, Callback cb, Interpolator interpolator) {
        if (forParent == null) {
        ...
        this.animationMultiplier = getAnimationDurationMultiplier(context);
        ...
        mScroller = ScrollerCompat.create(context, interpolator == null ? sInterpolator : interpolator);
    }

    private static float getAnimationDurationMultiplier(Context context) {
     ...
     * @param duration duration of animation, use -1 for automatic computation
    ...
    public boolean smoothSlideViewTo(View child, int finalLeft, int finalTop, int duration) {
        ...
        if (duration != -1) {
            duration = (int) (animationMultiplier * duration);

            continueSliding = forceSettleCapturedViewAt(finalLeft, finalTop, duration);
        } else {
            continueSliding = forceSettleCapturedViewAt(finalLeft, finalTop, 0, 0);
        }
    ...
    private int computeSettleDuration(View child, int dx, int dy, int xvel, int yvel) {
        ...
        return (int) ((xduration * xweight + yduration * yweight) * animationMultiplier);
    }
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    if (index != -1) {
                        ...
                        if (!isPointerDown(mActivePointerId))
                            break;
